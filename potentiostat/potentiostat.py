...
def message_received(client, server, message):
    ...
    pstat.write_auto_gain(False)
    pstat.write_gain(Resistors.R_1M)


    print("Test CV")
    pstat.write_switch(SwitchState.On)
    results = pstat.perform_CV(min_V = -1.5,
                               max_V = 0.5,
                               cycles= 5,
                               mV_s = 500,
                               step_hz=250
                               )
    pstat.write_switch(False)
    # print(results)

    filename = "CV_" + filename_core
    np.savetxt(filename, results, delimiter=",", fmt='%.3E,%.3E,%.3E,%d,%d,%.3E')
    print("CV done:", filename)
    os.system(f"python plot_CV.py {filename}")
    print("-------------")

    print("Test CDPV")
    results = pstat.perform_CDPV(min_V=-1.0,
                                 pulse_V=0.11,
                                 step_V=0.01,
                                 max_V=-0.2,
                                 potential_hold_ms=500,
                                 pulse_hold_ms=50,
                                 voltage_hold_s=5
                                 )
    pstat.write_switch(False)
    filename = "CDPV_" + filename_core
    np.savetxt(filename, results, delimiter=",", fmt='%.3E,%.3E,%.3E,%d,%d,%.3E')
    print("CDPV done:", filename)
    os.system(f"python plot_CDPV.py {filename}")
    print("-------------")
